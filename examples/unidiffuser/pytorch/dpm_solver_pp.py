...
def interpolate_fn(x: torch.Tensor, xp: torch.Tensor, yp: torch.Tensor) -> torch.Tensor:
    ...
    start_idx = torch.where(
        ...
        torch.where(torch.eq(x_idx, num_x_points), torch.tensor(num_x_points - 2, device=x.device), cand_start_idx),
    )
    ...
    def dpm_solver_second_update(self, x, s, t, r1=0.5, noise_s=None, return_noise=False, solver_type='dpm_solver'):
        ...
        if self.predict_x0:
            ...
            if solver_type == 'dpm_solver':
                ...
                x_t = ((sigma_t / sigma_s)[(...,) + (None,)*dims] * x - (alpha_t * phi_1)[(...,) + (None,)*dims] * noise_s + (1. / r1) * (alpha_t * ((torch.exp(-h) - 1.) / h + 1.))[(...,) + (None,)*dims] * (noise_s1 - noise_s))
            else:
